// import {ImageFrame, ImageItem} from "..";
// import {ImageUtil} from "./ImageUtil";
// import {APPLICATION_VERSION} from "./AppUtil";
//
describe.skip("ImageUtil", () => {

    test.todo("still needed?");

//    test("retrieves empty ImageItem, current version", () => {
//         const imageItem1 : ImageItem = ImageUtil.getEmptyImageItem();
//         const imageItem2 : ImageItem = ImageUtil.EMPTY_IMAGE_ITEM;
//
//         expect(imageItem1).toStrictEqual(imageItem2);
//    });
//
//     test("retrieves empty ImageItem, version 0.3.0", () => {
//         const imageItem1 : ImageItem = ImageUtil.getEmptyImageItem(APPLICATION_VERSION.V0_3_0);
//         const imageItem2 : ImageItem = ImageUtil.EMPTY_IMAGE_ITEM;
//
//         expect(imageItem1).toStrictEqual(imageItem2);
//     });
//
//     test("retrieves empty ImageItem, version 0.2.0", () => {
//         const imageItem1 : ImageItem = ImageUtil.getEmptyImageItem(APPLICATION_VERSION.V0_2_0);
//         const keys1 = Object.getOwnPropertyNames(imageItem1);
//         const keys2 = [
//             'filename',
//             'filetype',
//             'fullpath',
//             'width',
//             'height',
//             'src',
//             'guid',
//             'hashSHA256',
//             'hashMD5',
//             'frames',
//             'populateFrameDataComplete',
//             'filterAppliedAliasHash',
//             'filterAppliedTrimRect',
//             'filterAppliedPaddingInner',
//             'isEmpty',
//         ];
//
//         const result1 = keys2.every((key) => {
//             return keys1.includes(key);
//         });
//
//         const result2 = keys1.every((key) => {
//             return keys2.includes(key);
//         });
//
//         expect(result1).toEqual(true);
//         expect(result2).toEqual(true);
//     });
//
//     test("retrieves empty ImageFrame", () => {
//         const imageFrame : ImageFrame = ImageUtil.EMPTY_IMAGE_FRAME;
//
//         expect(imageFrame.width).toEqual(0);
//         expect(imageFrame.height).toEqual(0);
//         expect(imageFrame.isValid).toEqual(false);
//     });
//
//     // TODO: test ProjectUtil.populateImageFrames for all image types
//
//
//
//     // test("Creates ImageItem from DataURL", () => {
//     //     const imageDataURL = 'data:image/png;base64,';
//     //     const imageItem = ImageUtil.createImageItem('happy.png', imageDataURL);
//     //     const png = new ImageUtil_PNG(ImageFormat.PNG, imageDataURL);
//     //     const imageItem = png.buildImageItem(
//     //         FileUtil.getFileParts('./sprites/hero/happy.png') ?? FileUtil.EMPTY_FILEPARTS,
//     //         png.getFileBytes(imageDataURL) ?? ndarray([]),
//     //     );
//     //
//     //     expect(imageItem?.frames?.length).toEqual(1);
//     //     expect(imageItem?.frames?.flat()[0].data?.length).toEqual(96 * 96 * 4);
//     //     expect(imageItem?.frames?.flat()[0].width).toEqual(96);
//     //     expect(imageItem?.frames?.flat()[0].height).toEqual(96);
//     // });

});
